...
def show_order_forecast():
    ...
    #     ),
    #     hovermode="x unified",
    #     width=1200,  # equivalent to figsize=(12, ...)
    #     height=400,  # equivalent to figsize=(..., 4)
    #     margin=dict(r=150)  # make room for legend
    # )

    fig.update_layout(
    title={
        'text': '30-Day Order Volume Forecast Comparison',
        'y':0.95,
        'x':0.5,
        'xanchor': 'center',
        'yanchor': 'top',
    ...
    ##### Expected Order Volume Range Table #####
    st.subheader("Expected Order Volume Range")
    ...
    # forecast_table = data_manager.generate_forecast_table(forecast_df).rename(
    #     columns={
    #         'Forecast': 'Expected Orders',
    #         'Lower CI': 'Worst Case',
    #         'Upper CI': 'Best Case'
    ...
    styled_table = (
        ...
        .format({'Expected Orders': '{:,.0f}', 
                'Worst Case': '{:,.0f}', 
                'Best Case': '{:,.0f}'})
        .applymap(color_demand, subset=['Expected Orders'])
        )
    ...
    st.markdown(f"""
    **At a Glance:**
    - Typical expected volume: **{int(forecast_table['Expected Orders'].mean()):,} orders/day**  
    ...
    st.dataframe(
        styled_table_2,
        column_config={
            'Order Volume': st.column_config.TextColumn("🛒 Order Volume", width="auto"),
    ...
    metrics_table = {
        ...
        'Values': ['30', '126', '93', '70'],
        'Interpretation': [
            'On average, predictions are off by 30 orders/day.',
            'Prediction ranges are 126 orders wide (e.g., 100–226).',
            '93% of actual sales fall within the predicted range.',
            '70% of trend predictions (up/down vs. yesterday) are correct.'
        ]
    }
    ...
    # # Data Loading
    # data = data_manager.get_combined_data('order_volume')
    
    # if not data.empty:
    #     # Model Loading
    #     model, transformer = data_manager.load_model('order_volume')
    #     forecast = data_manager.generate_forecast(model, transformer, data)


    #     # Forecasting (Placeholder logic)
    #     last_date = data['date'].iloc[-1]
    #     forecast_dates = pd.date_range(start=last_date, periods=30)
    #     forecast_values = [data['y'].iloc[-1] * (1.02 ** i) for i in range(30)]
    ...
    #     # Visualizations
    #     fig1 = visualization.plot_historical_forecast(data, 'order_volume')
    #     st.plotly_chart(fig1, use_container_width=True)
        
    #     fig2 = visualization.plot_forecast_only(data, 'order_volume')
    ...
    #         styled_table_2,
    #         column_config={
    #             'Order Volume': st.column_config.TextColumn("🛒 Order Volume", width="medium"),
    #             'Forecast': st.column_config.TextColumn("📈 Forecast Condition", width="medium"),
    #             'Primary Action': st.column_config.TextColumn("✅ Priority Action", width="auto"),
    #             'Secondary Action': st.column_config.TextColumn("⚡ Next Step", width="auto")
    #         },
    #         hide_index=True,
    ...
    #     metrics_table = {
    #         'Metrics': ['Mean Abs. Error (MAE)', 'Prediction Width (PIW)', 'Prediction Coverage (PIC)', 'Trend Accuracy (MDA)'],
    #         'Values': ['25', '126', '93', '70'],
    #         'Interpretation': [
    #             'On average, predictions are off by 25 orders/day.',
    #             'Prediction ranges are 126 orders wide (e.g., 100–226).',
    #             '93% of actual sales fall within the predicted range.',
    #             '70% of trend predictions (up/down vs. yesterday) are correct.'
    #         ]
    ...
    #         df_metrics,
    #         column_config={
    #             'Metrics': st.column_config.TextColumn("📊 Metric", width="small"),
    #             'Values': st.column_config.TextColumn("🎯 Value", width="small"),
    #             'Interpretation': st.column_config.TextColumn("📝 Interpretation", width="large")
    #         },
    #         hide_index=True,
    ...
    #     # # Date selector for condition table
    #     # min_date = forecast_df['ds'].min()
    #     # max_date = forecast_df['ds'].max()
    #     # selected_date = st.date_input(
    #     #     "Select date to view recommendations",
    #     #     min_value=min_date,
    #     #     max_value=max_date,
    #     #     value=min_date
    #     # )
        
    #     # action_table = data_manager.generate_condition_table(
    #     #     'order_volume', 
    #     #     forecast_df
    #     #     # forecast_df,
    ...
    #     # Download Button
    #     # data_manager.download_button(forecast_df, 'order_volume_forecast.csv')
    # else:
